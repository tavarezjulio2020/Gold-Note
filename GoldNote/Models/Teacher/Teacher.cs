...
namespace GoldNote.Models.Teacher
{
    ...
    public class AssignmentDetails
    {
        ...
        public string Description { get; set; } // Correct C# spelling
    ...
    public class Teacher
    {
        ...
        public List<StudentDashboardItem> getMyStudents(string userId)
        {
            ...
            string sql = @"
                Select s.profile_id,
                 ...
                 GROUP BY s.profile_id, s.name, i.inst_name, li.learn_id";
        ...
        public List<AssignmentDetails> GetAssignments(int studentLearnId)
        {
            ...
            while (reader.Read())
            {
                list.Add(new AssignmentDetails
                {
                    ...
                    // Map DB typo 'desctription' to correct C# prop 'Description'
                    Description = reader["desctription"].ToString(),
                    TimesPracticedThisWeek = Convert.ToInt32(reader["PracticeCount"]),
                    LearnId = studentLearnId
                });
            }
            return list;
        }

        public void AddAssignment(int learnId, string title, string description, string teacherId)
        {
            using var con = new SqlConnection(_connectionString);
            con.Open();
            ...
            try
            {
                // 1. Insert into assignment table INCLUDING creator_id
                // We parse the string teacherId into a GUID
                string sqlInsertAssignment = @"
            INSERT INTO assignment (title, desctription, creator_id, creationDate) 
            OUTPUT INSERTED.assignment_id 
            VALUES (@Title, @Description, @CreatorId, GETDATE());";

                int newAssignmentId;
                using (var cmdAssign = new SqlCommand(sqlInsertAssignment, con, transaction))
                {
                    cmdAssign.Parameters.AddWithValue("@Title", title);
                    cmdAssign.Parameters.AddWithValue("@Description", description);
                    // Ensure we handle the GUID correctly
                    cmdAssign.Parameters.Add("@CreatorId", System.Data.SqlDbType.UniqueIdentifier).Value = Guid.Parse(teacherId);

                    newAssignmentId = (int)cmdAssign.ExecuteScalar();
                }

                // 2. Link to student
                string sqlAssignStudent = @"
            INSERT INTO assigned_assignments (assignment_id, learn_id) 
            VALUES (@AssignmentId, @LearnId);";

                using (var cmdAssignStudent = new SqlCommand(sqlAssignStudent, con, transaction))
                {
                    cmdAssignStudent.Parameters.AddWithValue("@AssignmentId", newAssignmentId);
                    cmdAssignStudent.Parameters.AddWithValue("@LearnId", learnId);
                    cmdAssignStudent.ExecuteNonQuery();
                }
        ...
        public void UpdateAssignment(int assignmentId, string title, string description)
        {
            // NOTE: 'desctription' matches your DB column typo
            string sqlUpdate = @"
                UPDATE assignment
        ...
        // --- Class Code & Request Logic ---

        public classCode GetClassCode(string userId)
        {
            classCode data = null;
            string sql = "SELECT join_Code FROM classRoom WHERE teacher_id = @teacherID";
            ...
            if (reader.Read())
            {
                data = new classCode { ClassCode = reader["join_Code"].ToString() };
            }
            return data;
        }
        ...
        {
            string sql = "SELECT classRoom_id FROM classRoom Where teacher_id = @teacherID";
            using var con = new SqlConnection(_connectionString);
            using var cmd = new SqlCommand(sql, con);
            cmd.Parameters.AddWithValue("@teacherID", teacherID);
            con.Open();
        ...
        {
            string sql = "INSERT INTO studentInClass (classroom_id, student_instrument_id) VALUES (@classRoomId, @student)";
            using var con = new SqlConnection(_connectionString);
            using var cmd = new SqlCommand(sql, con);
        ...
        {
            string sql = "DELETE FROM classRoomRequest WHERE crrID = @requestId";
            using var con = new SqlConnection(_connectionString);
            using var cmd = new SqlCommand(sql, con);
        ...
        public string CreateClassroom(string teacherId, string classroomName, string selectedPlan = "Promotional")
        {
            ...
            using (var con = new SqlConnection(_connectionString))
            {
                ...
                // Get the INT account_id from the profile table
                int accountIdInt = 0;
                string sqlGetAccountId = "SELECT account_id FROM profile WHERE profile_id = @ProfileGuid";
                using (var cmdLookup = new SqlCommand(sqlGetAccountId, con))
                {
